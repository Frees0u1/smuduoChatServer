...
//此类型的构造函数尚未实现
InetAddress::InetAddress(smuduo::StringArg ip, uint16_t port)
{
    bzero(&saddr_, sizeof saddr_);
    smuduo::sockets::fromHostPort(ip.c_str(), port, &saddr_);
}
...
bool InetAddress::resolve(smuduo::StringArg hostname, InetAddress* out){
    assert(out);
